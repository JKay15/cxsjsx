...
GameObject BleedSpell() {
    ...
    spell.addBehaviour()->onCollision = [&](GameObject* target) { // 添加碰撞处理函数
...
GameObject Skeleton() {
    ...
    unit.behaviours.push_back(new March(unit, 16)); // 添加行军行为
    return unit; // 返回游戏对象
}
...
GameObject TheKing() {
    ...
    boss->onDamage = [&](struct Damage* dmg) { // 设置自定义行为的受伤处理函数
    ...
    summons->changeOnSummon([&]() { // 设置召唤行为的召唤处理函数
...
GameObject ShellKnight() {
    ...
    shell->onUpdate = [&]() { // 设置自定义行为的更新处理函数
        ...
        shell->sprite = shelled ? sprites::status_shielded : std::vector<int>(); // 根据护盾状态更改行为精灵图像（用于显示护盾状态）
        return std::optional<bool>();
    };

    shell->onDamage = [&](struct Damage* dmg) { // 设置自定义行为的受到伤害处理函数
...
GameObject RageKnight() {
    ...
    std::optional<March*> marchOptional = unit.getBehaviour<March>();
    March* march=nullptr;
    if (marchOptional.has_value()) {
        march = marchOptional.value();
        // 在这里可以使用 `march` 指针
    }// 获取行军行为对象指针
    Enraged* enraged = new Enraged(unit, SPELL); // 创建愤怒行为对象（对法术免疫）
...
GameObject RoyalGuard() {
    ...
    shield->onDamage = [&](struct Damage* dmg) { // 设置自定义行为的受伤处理函数
    ...
    std::reverse(unit.behaviours.begin(), unit.behaviours.end());
    // 反转行为列表顺序，使盾牌行为优先执行
...
GameObject Portal() {
    ...
    std::vector<GameObject(*)()> tmp={&Villager, &Bandit, &Archer};
    Summon* summonBehaviour = new Summon(unit, [&]() { return randomElement(tmp)(); }, 3000);
